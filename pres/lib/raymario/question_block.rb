class QuestionBlock < Block
  ...
  attr_accessor :coinAnimationTime, :coinAnimationAcum, :coinFrameAcum, :coinAnimationFrame,
                :coinAnimationRunning, :coinY, :coinVelY, :stardustAnimationTime,
                :stardustAnimationAcum, :stardustAnimationFrame, :maxStartDustAnimationFrame,
                :stardustAnimationRunning, :pointsFrameAcum, :pointsFrameTime, :pointsAnimationRunning
  ...
  def draw
    ...
    if @hit
      ...
      DrawTexture(textures["blockQuestion#{@currentFrame}"], @pos.x, @pos.y, WHITE)
    end
